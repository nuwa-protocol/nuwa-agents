...
export default function AiMenu({
  ...
  useEffect(() => {
    ...
    try {
      ...
      let dark = false;
      while (el) {
        if (el.classList && el.classList.contains("dark")) {
          dark = true;
          break;
        }
        el = el.parentElement;
  ...
  const suggestions =
    variant === "generate" ? generateSuggestions : transformSuggestions;
        ...
        <div className="flex flex-col gap-2">
          ...
          {(aiResult || isStreaming) && (
            <div className="max-h-48 overflow-auto text-sm whitespace-pre-wrap border border-gray-100 dark:border-gray-800 rounded-md p-2 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
              {aiResult}
              {isStreaming && (
                <span className="ml-1 text-gray-400 dark:text-gray-400 animate-pulse">
                  ▍
                </span>
              )}
            </div>
