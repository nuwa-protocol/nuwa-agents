...
const ElementUpdateSchema = z.object({
	...
	props: z
		.object({
			...
			strokeStyle: z.enum(["solid", "dashed", "dotted"]).optional(),
			fillStyle: z
				.enum(["solid", "hachure", "zigzag", "cross-hatch"])
				.optional(),
			strokeWidth: z.number().optional(),
		})
		.strict(),
	...
	useEffect(() => {
		...
		server.registerTool(
			"get_scene",
			{
				title: "Get Scene",
				description:
					"Return full scene: elements (incl. deleted), appState, files",
				inputSchema: {},
			},
			async () => {
		...
		server.registerTool(
			"add_elements",
			{
				title: "Add Elements",
				description:
					"Add one or more elements. Uses convertToExcalidrawElements for convenience",
		...
		server.registerTool(
			...
			async ({ updates }) => {
				...
				if (!list.length)
					return jsonContent({ success: false, reason: "No updates provided" });
		...
		server.registerTool(
			...
			async ({ ids }) => {
				...
				const next = api
					.getSceneElements()
					.filter((e: any) => !toRemove.has(e.id));
				api.updateScene({ elements: next as any });
